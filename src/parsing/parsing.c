...
static int handle_special_commands(char *buffer, labyrinth_t *maze,
    ROOM_PARSING_STATUS_t *room_status_flag)
{
    ...
    if (my_strcmp(buffer + 2, "end") == 0) {
        ...
        *room_status_flag = END;
        ...
        handle_unknown_command(buffer);
    }
    return 0;
}

static int parse_hashtag(char *buffer, labyrinth_t *maze,
    ROOM_PARSING_STATUS_t *room_status_flag)
{
    ...
    return handle_special_commands(buffer, maze, room_status_flag);
}
...
static int parse_room(char *buffer, labyrinth_t *maze,
    ROOM_PARSING_STATUS_t room_status_flag)
{
    int x;
    int y;
...
static int parse_tunnel(char *buffer, char *current_line, labyrinth_t *maze)
{
    if (!maze->end || !maze->start) {
        my_putstr("GTFO - should not read tunnels before the "
            "starting/ending room has been read.");
        return 84;
    }
...
static int parse_room_or_tunnel(char *buffer, labyrinth_t *maze,
    ROOM_PARSING_STATUS_t room_status_flag)
{
...
static int init_labyrinth_with_num_robots(char *buffer, size_t *bufsize,
    labyrinth_t *maze)
{
...
static int process_labyrinth_input(labyrinth_t *maze, char **buffer,
    size_t *bufsize)
{
    ssize_t read;
...
labyrinth_t *read_labyrinth(void)
{
    ...
    if (process_labyrinth_input(maze, &buffer, &bufsize) == 84) {
        free_labyrinth(maze);
        free(buffer);
        return NULL;
    }
